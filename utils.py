...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        dst_train = datasets.ImageFolder("/data/runkai/tiny-imagenet-200/train", transform=transform) # no augmentation
        dst_test = datasets.ImageFolder("/data/runkai/tiny-imagenet-200/test", transform=transform)
        ...
        std = [0.5, 0.5, 0.5]
        transform = transforms.Compose([transforms.Resize(im_size), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.ImageFolder("/data/rzheng/imagenette2-160/train", transform=transform) # no augmentation
        dst_test = datasets.ImageFolder("/data/rzheng/imagenette2-160/val", transform=transform)
        class_names = dst_train.classes
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    elif model == 'ResNet50GN':
        net = timm.create_model('resnet50_gn', pretrained=False)
    elif model == 'ResNet50GN_pretrained':
...
def convert_rgb_to_grayscale_and_resize(tensor, target_size=(28, 28)):
    """
    Convert a batch of RGB images to grayscale and resize them.
    ...
    Returns:
        torch.Tensor: Output tensor of shape (N, 1, H_out, W_out).
    """
    if tensor.ndim != 4 or tensor.size(1) != 3:
        raise ValueError("Input tensor must have shape (N, 3, H, W)")
    ...
    # Convert to grayscale by weighted sum
    grayscale = (tensor * weights).sum(dim=1, keepdim=True)  # Shape: (N, 1, H, W)
    
    # Resize using bilinear interpolation
    resized = F.interpolate(grayscale, size=target_size, mode='bilinear', align_corners=False)
...
class Averager:
    def __init__(self, model_params, alpha=0.999):
        """
        Initializes the Averager.

        Args:
            model_params (iterable): Iterable of model parameters to average.
        ...
        for idx, param in enumerate(model_params):
            self.shadow[idx] = param.data.clone()

    def update(self, model_params):
        """
        Updates the averaged parameters.

        Args:
            model_params (iterable): Iterable of current model parameters.
        """
        for idx, param in enumerate(model_params):
            assert idx in self.shadow, "Mismatch in parameter indices."
            new_average = self.alpha * self.shadow[idx] + (1.0 - self.alpha) * param.data
            self.shadow[idx] = new_average.clone()

    def get_average_params(self):
        """
        Retrieves the averaged parameters.

        Returns:
            List[torch.Tensor]: List of averaged parameters.
        """
        return [param.clone() for param in self.shadow.values()]
